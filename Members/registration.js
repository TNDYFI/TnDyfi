  ...
  fields.forEach((section, sectionIndex) => {
    ...
    section.fields.forEach((field, fieldIndex) => {
      ...
      if (field.type === 'select') {
        ...
        inputHtml = `<input id="${fieldId}" type="${field.type}" name="${field.name}" ${field.required ? 'required' : ''} ${minAttr} ${maxAttr} ${valueAttr} placeholder="Enter ${field.label.toLowerCase()}">`;
      }
